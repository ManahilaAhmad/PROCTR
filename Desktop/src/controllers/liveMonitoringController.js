class LiveMonitoringController {
  constructor(violationModel) {
    this.model = violationModel;
    this.alertCount = 0;
  }

  handleSensorPayload(payload) {
    if (payload.type === 'SENSOR_SYSTEM_START') {
      this.updateWorkspaceInfo(payload.workspace_dir);
    } else if (payload.type === 'VIOLATION_ALERT') {
      const record = this.model.addViolation(payload);
      this.renderViolationCard(record);
    }
  }

  updateWorkspaceInfo(path) {
    const wsElem = document.getElementById('val-workspace');
    if (wsElem && path) {
      wsElem.innerText = path;
    }
  }
  ...
  renderViolationCard(v) {
    ...
    if (counterElem) counterElem.innerText = `${this.alertCount} Alert(s) Logged`;
...
module.exports = LiveMonitoringController;
